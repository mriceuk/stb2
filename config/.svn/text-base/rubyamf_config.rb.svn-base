...
module RubyAMF
  module Configuration
    ...
    #ClassMappings.register(:actionscript => 'vo.Person', :ruby => 'org.mypackage.Person')
    #
   ...
   # ClassMappings.register({:type=>"Hash",:actionscript => 'com.pokelondon.spotthebull.vos.model.VOGame', :ruby => 'VO::Game',:attributes => ["timetogoinseconds"]})
    ...
    # Here's an example:
    # ParameterMappings.register(:controller => :UserController, :action => :find_friend, :params => { :friend => "[0]['friend']" })
    ParameterMappings.register( {:controller => :FieldController, :action => :getGameDetails, :params => {:datePlayed => "[0]", :email=>"[1]" } })
    ParameterMappings.register( {:controller => :FieldController, :action => :getGameDetailsWithInvitation, :params => {:invitation => "[0]" } })
    ParameterMappings.register( {:controller => :FieldController, :action => :getGameDetailsWithDateAndEmailAndUid, :params => {:date => "[0]", :email=>"[1]",:uid=>"[2]" } })

    ParameterMappings.register( {:controller => :FieldController, :action => :inviteFriends, :params => {:uid => "[0]", :friends=>"[1]" } })
    ParameterMappings.register( {:controller => :FieldController, :action => :createLocation, :params => {:x => "[0]", :y=>"[1]", :square=>"[2]" } })



    ParameterMappings.register( {:controller => :FieldController, :action => :createEntrant, :params => {:name=>"[0]",:email=>"[1]", :mobile=>"[2]", :square=>"[3]", :optin=>"[4]", :feed=>"[5]"}})
    ParameterMappings.register( {:controller => :FieldController, :action => :confirmSingleEntrant, :params => {:uid=>"[0]"}})


    ParameterMappings.register( {:controller => :FieldController, :action => :acceptInvitationAndUpdateEntrant, :params => {:uid => "[0]", :name=>"[1]", :mobile=>"[2]", :square=>"[3]", :optin=>"[4]", :feed=>"[5]"}})
 
    # ParameterMappings.register( {:controller => :FieldController, :action => :getPlayingActivityMapForDate, :params => {:date => "[0]"}})
    # ParameterMappings.register( {:controller => :FieldController, :action => :getBullActivityMapForDate, :params => {:date => "[0]"}})
    ParameterMappings.register( {:controller => :FieldController, :action => :getPlayingActivityMap, :params => {:period => "[0]"}})
    ParameterMappings.register( {:controller => :FieldController, :action => :getBullActivityMap, :params => {:period => "[0]"}})
    ParameterMappings.register( {:controller => :FieldController, :action => :getBullLocationsForDate, :params => {:date => "[0]"}})
